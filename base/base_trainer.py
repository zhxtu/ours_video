...
class BaseTrainer:
    def __init__(self, model, resume, config, iters_per_epoch, train_logger=None, test_logger=None, gpu=None, test=False):
        self.model = model
        self.config = config
        ...
        if gpu == 0:
            ...
            if test is True:
                log_dir = os.path.join('test/', config['experim_name'], date_time)
            else:
                log_dir = os.path.join(config['trainer']['log_dir'], config['experim_name'], date_time)
        ...
        if self.gpu == 0:
            if test is False:
                ...
                self.checkpoint_dir = os.path.join(cfg_trainer['save_dir'], run_name,date_time)
                # if not dir_exists(self.checkpoint_dir):
                ...
                writer_dir = os.path.join(cfg_trainer['log_dir'], run_name,date_time)
