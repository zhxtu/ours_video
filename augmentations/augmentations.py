...
class Compose(object):
    ...
    def __call__(self, imgs, mask=None):
        ...
        for a in self.augmentations:
            imgs_, parm = a(imgs_)
            aug_parm.append(parm)
        return imgs_, aug_parm


class RandomCrop(object):
    def __init__(self, sizes, padding=0):

        size, start_point=sizes
        self.size = int(size)
    ...
    def __call__(self, imgs):
        ...
        for (idx, img) in enumerate(imgs):
            ...
            w, h = img.size
            th, tw = int(self.size), int(self.size)
            if w == tw and h == th:
                imgs_.append(img)
            else:
                if w < tw or h < th:
                    ...
                    if idx==0:
                        x1 = self.start_point[0] #random.randint(0, w - tw)
                        y1 = self.start_point[1] #random.randint(0, h - th)
                    img = img.crop((x1, y1, x1 + tw, y1 + th))
                    imgs_.append(img)
        return imgs_, [self.size,self.start_point]

class RandomResizedCrop(object):
    def __init__(self, sizes, padding=0):

        cropsize, start_point, scale =sizes
    ...
    def __call__(self, imgs):
        ...
        for (idx, img) in enumerate(imgs):
            ...
            x1 = self.start_point[0]  # random.randint(0, w - tw)
            y1 = self.start_point[1]  # random.randint(0, h - th)
            if self.cropsize == crop_w and self.cropsize == crop_h:
                img = img.crop((x1, y1, x1 + crop_w, y1 + crop_h))
                imgs_.append(img)
            else:
                img = img.crop((x1, y1, x1 + crop_w, y1 + crop_h))
                img = img.resize([self.cropsize, self.cropsize], Image.BILINEAR)
                imgs_.append(img)
        return imgs_, [self.start_point, crop_w, self.cropsize]
...
class ColorJitter(object):
    ...
    def __call__(self, imgs):
        ...
        for (idx, img) in enumerate(imgs):
            ...
            imgs_.append(img)
        return imgs_, [r_brightness, r_contrast, r_saturation]
