...
class TrainerSeq(BaseTrainer):
    def __init__(self, model, resume, config, supervised_loader, unsupervised_loader, iter_per_epoch,
        ...
        super(TrainerSeq, self).__init__(model, resume, config, iter_per_epoch, train_logger, gpu=gpu, test=test)
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx in tbar:
            ...
            if self.mode == 'supervised':
                (input_l, target_l),  input_ul = next(dataloader), None

            else:
                (input_l, target_l), (input_ul, ul1, br1, ul2, br2, flip) = next(dataloader)
                ...
                for i in range(2):
                    ...
                    ul2[i] = ul2[i].flatten()
                    br2[i] = br2[i].flatten()
                    flip[i] = flip[i].flatten()
                input_ul=input_ul.flatten(0, 1)
                input_ul = input_ul.cuda(non_blocking=True)
